...
public class DashboardMapper {
    ...
    public Map<String, Object> getBigScreenData() {
        ...
        // === 各网格事件排名（以二级网格为基准：三级网格事件归入其父网格；字段名 count 与前端对齐） ===
        ...
        if (unassigned != null && unassigned > 0) {
            ...
            row.put("gridName", "未分配网格");
            row.put("count", unassigned);
            gridRanking.add(row);
            gridRanking.sort((a, b) -> Long.compare(((Number) b.get("count")).longValue(), ((Number) a.get("count")).longValue()));
        }
