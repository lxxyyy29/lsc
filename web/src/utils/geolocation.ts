/**
 * Web 管理端定位工具：优先浏览器原生定位（navigator.geolocation，仅 HTTPS 安全上下文可用），
 * 失败时回退高德 IP 定位（城市级大致位置）。与 H5 端 h5/src/utils/geolocation.ts 策略一致。
 */
...
/**
 * 获取当前位置：浏览器原生定位(HTTPS) → 高德 IP 定位；全部失败时抛出异常
 */
