  ...
  <div>
    ...
    <div v-if="activeTab === 'waylines'" class="card">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">
        ...
        <span style="font-size:12px;color:#9ca3af;">航线由无人机平台侧创建，本平台仅同步展示</span>
      </div>
      <table class="table">
...
// 航线由无人机平台侧创建，本平台只做查询代理：
// 上游开放平台只提供「航线列表」「航点坐标」两个 GET 接口，没有「新建航线」，
// 因此这里不再提供新增入口（原实现调用 POST /drone/waylines 必然失败）。
