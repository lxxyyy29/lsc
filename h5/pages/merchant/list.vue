<template>
  <view class="page">
      ...
      >
        <view class="card-left">
          <view class="photo-thumb">
            <image v-if="item.merchantPhotoUrl" :src="toImageUrl(item.merchantPhotoUrl)" class="thumb-img" mode="aspectFill" />
            <AppIcon v-else name="briefcase" size="32rpx" class="thumb-icon" />
          </view>
        </view>
        <view class="card-body">
          ...
          <view v-if="item.areaName" class="info-row">
            <AppIcon name="location" size="22rpx" class="info-icon" />
    ...
    <view v-else-if="!loading" class="empty-state">
      <text class="empty-title">暂无商户数据</text>
      <text class="empty-desc">{{ keyword || statusFilter ? '调整筛选条件后重试' : '点击右下角按钮新增商户' }}</text>
    </view>
...
import { listMerchants, type MerchantItem } from '../../src/api/merchant'
import { ensureAuthenticated, navigateToPath } from '../../src/uni/navigation'
import { hasButtonPermission, hasMenuPermission } from '../../src/auth/permissions'
import { toImageUrl, fetchAccessPrefix } from '../../src/api/upload'

const allList = ref<MerchantItem[]>([])
...
const canCreate = computed(() => hasButtonPermission('button:h5:merchant:create'))
...
async function loadData() {
  ...
  try {
    allList.value = await listMerchants()
  } catch {
...
function openDetail(id: number) {
  navigateToPath(`/merchants/${id}`)
}

function openCreate() {
  navigateToPath('/merchants/create')
}
